...
- Status: **Resolvido (2026-09-24)** — fechado a pedido do usuário, sem implementar M1, M2, M4 e M6. Idempotência/saga ficam como dívida aceita (SDD R-03; consistência eventual por fila + reconciliação, ADR-005/006), sem estorno no contrato v1.0 e sem confirmação da C3 pelo time C#. Histórico anterior: **Aberto (adiado)** — retomar no fim do projeto, com todas as tarefas prontas, antes do encerramento/deploy. Atualização 2026-09-24: o item M5 (falha de `Enfileirar` do e-mail engolida) foi coberto pela RF12-01 e parte do M3 pela RF13-02 (pós-quitação no reenvio); M1 (mensagem da UI quando o Financeiro quitou e a gravação local falhou), M2, M4 e M6 continuam abertos.

## Bloqueio 007 — 2026-09-24
- Reportado por: orquestrador do `/executar --continuar` (após fechar todas as Refatorações que não dependem de IDE)
- Escalado para: usuário
- Artefato/trecho afetado: `TASK.md` — Lote 14 (T54, T55), Lote 16 (T59, T60, T61, T63, T64), Lote 17 (T66) e as tarefas de Refatoração RF11-07, RF11-08, RF12-07, RF12-09, RF13-10; nada do que foi implementado hoje foi compilado nem executado
- Descrição: o ambiente do Executor não tem compilador Delphi, IDE, Report Designer, Firebird em execução, SMTP real nem o Financeiro C#. Todo o código, layout (`PedidoLayout.dfm`), SQL (`db/01_schema.sql` com `CREATE COLLATION`) e testes escritos desde as RF do Lote-4 até as RF13/RF15 foram validados apenas por leitura (QA-REPORT/SECURITY-REVIEW) e estão com "não compilado nem executado" nas notas. Restam pendentes por exigirem ambiente real: RF11-07 (P1, conferir no Report Designer cabeçalho, WordWrap/Stretch, descrição longa e paginação; bloqueia T54), RF11-08 (formato pt-BR de data e moeda independente do locale, confirmar `hh:nn` no DisplayFormat), RF12-07 (P1, conferir hostname CN/SAN do certificado SMTP; depende da API de SAN do `TIdX509`; bloqueia T54), RF12-09 (confirmar assinatura de `OnVerifyPeer`, exceção sem STARTTLS, testes de `RemetenteValido`/`Enviar`), RF13-10 (confirmar compilação dos testes e da UI de Pendências; bloqueia T54), e os Lotes 14, 16 e 17 (T54/T55 dependem do C# real ou do mock rodando com a IDE; T59-T64 são execução de roteiro, build Release, gbak e instalação em pasta limpa; T60 e T66 dependem de abrir o app). A T62 está sendo tratada por outra sessão (worktree `executar_tarefa-T62`).
- Impacto se não resolvido: risco alto de acumular erros de compilação e de comportamento (o primeiro build de tudo isso ainda não aconteceu); o smoke T54, o pacote de entrega (T61-T64) e a entrega de 25/09 não podem avançar; `/executar` não tem mais tarefa elegível sem esse ambiente (as elegíveis restantes são só T65, opcional/P2, que não é recomendável antes do primeiro build).
...
- Status: **Resolvido (2026-09-24)** — fechado a pedido do usuário. Todas as tarefas afetadas (T54-T66, RF11-07/08, RF12-07/09, RF13-10, RF14-05) constam `Concluída` no `TASK.md`. Itens listados abaixo como "não verificados" em ambiente real (banco UTF8, TLS/`CaFile`, timeout/5xx/409 contra o C#) ficam como risco residual aceito pelo usuário.
...
- Status: **Resolvido (opção 1)** — T61 Concluída em 2026-09-24: Release compilou, `montar-bin.ps1` saiu OK e o exe rodou fora da IDE (validado pelo usuário).
