...
| T08 | Domínio: `TCliente/TProduto/TVenda/TVendaItem`, enums `TStatusVenda/TTipoFila`, resultados tipados, as 7 interfaces (assinaturas) | Executor(Delphi) | SUG | P0 | ADR-001/010, SDD §2 | 4 | T07 | T09, T10 | Compila sem `uses` de Vcl/FireDAC/System.Net/Id* nas units de Domínio (checar cláusulas `uses`); status literais = 'Pendente','Quitada','Cancelada' | Em andamento — criadas `src/Dominio/Entidades/ERPV.Dominio.{Cliente,Produto,Venda,VendaItem,Enums,Resultados}.pas` (entidades com campos batendo com `db/01_schema.sql`; `Currency` em todo valor monetário; `TStatusVenda`/`TTipoFila` com conversão de/para os literais exatos `'Pendente'/'Quitada'/'Cancelada'` e `'QUITACAO'/'CANCELAMENTO'/'EMAIL'`; `TResultadoFinanceiro`/`TResultadoEnvioEmail` como record+enum, sem lib de terceiros) e as 7 interfaces em `src/Dominio/Contratos/ERPV.Dominio.Contratos.I*.pas` (`IClienteRepository`, `IProdutoRepository`, `IVendaRepository`, `IFilaRepository`, `IFinanceiroGateway`, `IEmailSender`, `IRelatorioPedido`), com métodos derivados dos critérios de aceite de T17/T21/T25-26/T37/T34-36/T48/T47. `ERPVendas.dproj`: `DCC_UnitSearchPath` ganhou `src\Dominio\Entidades` (pasta nova, unit não referenciada via `in` no `.dpr`); `.dpr` não mudou (nenhuma unit de Domínio é instanciada por ele). Verificação manual da cláusula `uses` de todas as 13 units de `src/Dominio` (grep dedicado): nenhuma referencia `Vcl.*`/`FireDAC.*`/`System.Net.*`/`Id*` — só `System.SysUtils`, `System.Generics.Collections`, `Data.DB` (exceção explícita do ADR-010 para `TDataSet`) e units internas `ERPV.Dominio.*`. Pendente confirmação de compilação na IDE pelo usuário (F9/Ctrl+F9, mesmo padrão de T07) antes de virar Concluída. |
| T09 | `ERPV.Core.Config` (TIniFile: conexão, URL, timeout, SMTP, pasta PDF, pasta log, ApiKey opcional, variável de ambiente) + `config/erpvendas.ini.example` | Executor(Delphi) | SUG | P0 | RNF-03/04, ADR-007/008 | 2 | T07 | T08, T10 | Valores lidos do INI de teste; `.ini.example` só com dados fictícios; INI ausente => mensagem clara e saída controlada | Em andamento — `src/Core/ERPV.Core.Config.pas` (TConfiguracao) + `config/erpvendas.ini.example` criados; senha SMTP/ApiKey/senha banco com precedência de variável de ambiente (ENV_SMTP_SENHA/ENV_FINANCEIRO_APIKEY/ENV_BANCO_SENHA); TimeoutSegundos lido do INI com `TimeoutMs` já convertido para uso futuro em T34 (achado do spike, docs/ambiente-licencas.md §4); INI ausente/seção-chave faltando lança `EConfiguracao` (Exception simples, T11 pode migrar p/ EInfra). Aguardando compilação/confirmação na IDE (Delphi 13 CE não compila via CLI, docs/ambiente-licencas.md §12) — não marcar Concluída até então. |
| T10 | `ERPV.Core.Log` (arquivo por dia, níveis, mascara CPF/CNPJ/e-mail, sem senha) | Executor(Delphi) | SUG | P1 | RF-24, ADR-008, SDD §7 | 1.5 | T07, T09 | T08 | Log criado na pasta configurada; CPF `12345678909` aparece mascarado; chamada com senha em texto não a grava | Em andamento — `src/Core/ERPV.Core.Log.pas` criado (`TLogger`, unit scope `ERPV.Core.Log`): recebe pasta de log via parâmetro simples no construtor (sem depender de `ERPV.Core.Config`, integração real com o valor do INI fica para T13, tratamento leve da dependência de T09 combinado nesta rodada); grava em `<pasta>\erpvendas-AAAAMMDD.log` (arquivo por dia, UTF-8, append), níveis INFO/AVISO/ERRO (`Info`/`Aviso`/`Erro`) conforme ADR-008; `MascararSensiveis` (class function, testável isoladamente) mascara CPF/CNPJ formatados ou só dígitos (`12345678909` -> `***.456.789-**`, padrão do próprio ADR-008) e e-mail (`usuario@dominio.com` -> `u***@dominio.com`), mais camada extra de redação de `senha=`/`password=`/`apikey=`/`token=`/`secret=`; não existe método público de "log de string crua sem mascarar" — todo caminho (Info/Aviso/Erro) passa pela máscara; falha de gravação nunca lança exceção (best effort, ADR-008). Instruções de verificação manual (código de teste pronto para colar num form/projeto temporário na IDE) documentadas no cabeçalho da própria unit. **Pendente de confirmação do usuário:** compilar na IDE (Delphi não compila via CLI nesta máquina, achado de T07) e rodar o roteiro manual do cabeçalho para confirmar arquivo criado, CPF mascarado na prática e senha nunca gravada — só então muda para `Concluída`. **Fix pós-tentativa de compilação real (coordenador forçou teste com `uses` temporário no FormMain):** erro `E2003 Undeclared identifier: 'TPath'` em `NomeArquivoDoDia` — `TPath` é de `System.IOUtils`, que não estava na cláusula `uses`; corrigido adicionando `System.IOUtils` ao `uses` da `implementation` (não precisa ir na `interface`, pois `TPath` só é usado internamente). Aguardando nova tentativa de compilação do usuário. |
